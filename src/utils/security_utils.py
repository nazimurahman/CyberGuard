...
class SecurityValidator:
    ...
    def validate_url(self, url: str, require_https: bool = True, 
        ...
        try:
            ...
            # 2. Check network location (domain/IP)
            if not parsed.netloc:
                return False, "Missing domain or IP"
            
            # 3. Validate domain or IP
            is_valid_domain = self._validate_network_location(parsed.netloc)
            if not is_valid_domain[0]:
                return is_valid_domain
            
            # 4. Check for path traversal in path
            ...
            # 6. Check against allowed domains if specified
            if allowed_domains:
    ...
    def _sanitize_html(self, html: str) -> str:
        ...
        # Allow safe tags only (in strict mode)
        if self.strict_mode:
            allowed_tags = {'b', 'i', 'u', 'strong', 'em', 'p', 'br', 'div', 'span'}
            # Simple tag stripping - in production use a proper HTML sanitizer like bleach
    ...
    def _sanitize_url(self, url: str) -> str:
        ...
        try:
            ...
            # If URL parsing fails, return encoded version
